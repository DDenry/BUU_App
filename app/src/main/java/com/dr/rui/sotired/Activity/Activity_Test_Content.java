...
import com.dr.rui.sotired.Fragment.Fragment_Instrument_Content;
import com.dr.rui.sotired.Fragment.Fragment_Test_Result;
import com.dr.rui.sotired.R;

public class Activity_Test_Content extends Activity implements Fragment_Instrument_Content.CallBackValue {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //
        setContentView(R.layout.activity_activity_test_content);
        //
        button_previous = (Button) findViewById(R.id.button_test_content_previous);
        button_next = (Button) findViewById(R.id.button_test_content_next);
    ...
    private class onClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.button_test_content_previous:
                    if (number > 0) {
                        //
                        hideFragment();
                        number--;
                    ...
                    break;
                case R.id.button_test_content_next:
                    number++;
                    //
                    hideFragment();
                    if (number < count) {
    ...
    private class loadResultFragment extends AsyncTask<Void, Void, Boolean> {
        ...
        protected void onPreExecute() {
            //super.onPreExecute();
            alertDialog = new AlertDialog.Builder(Activity_Test_Content.this).setTitle("系统消息").setMessage("正在计算结果……").create();
            alertDialog.show();
        }
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            fragmentTransaction = fragmentManager.beginTransaction();
            fragmentTransaction.add(R.id.linearLayout_test_content, arr[count], "Fragment" + count).commit();
            return true;
        }
    ...
    private class loadQuestion extends AsyncTask<Void, Void, Boolean> {
        ...
        protected void onPreExecute() {
            //super.onPreExecute();
            alertDialog = new AlertDialog.Builder(Activity_Test_Content.this).setTitle("系统消息").setMessage("正在加载……").create();
            alertDialog.show();
        }
        ...
        protected Boolean doInBackground(Void... params) {
            for (int i = 0; i < count; i++) {
                ...
                Fragment_Instrument_Content fragment_instrument_content = new Fragment_Instrument_Content();
                fragment_instrument_content.setArguments(bundle);
                arr[i] = fragment_instrument_content;
                fragmentManager = getFragmentManager();
                fragmentTransaction = fragmentManager.beginTransaction();
                fragmentTransaction.add(R.id.linearLayout_test_content, arr[i], "Fragment" + i).commit();
            }
            return true;
        }
